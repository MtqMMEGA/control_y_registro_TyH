...
$query = "SELECT fecha, temp, temp_max FROM registro_errores where fecha >= ".$fechaini." AND fecha <= ".$fechafin." AND temp>temp_max ORDER BY id";
$result = mysql_query($query) or die("Error en la instruccion SQL");
$puntos_grafica = array();
while($row = mysql_fetch_array($result))
{        
    $punto = array("label" => $row['fecha'] , "y" => $row['temp'] , "y2" => $row['temp_max']);
