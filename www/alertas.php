...
<html lang="en">
  <head>
    ...
    <title>Alertas Registradas</title>
    ...
    document.body.removeChild(link);
		...
		$(document).ready(function() {
			$('form').submit(function(e) {
				...
				var datosHumedad = [];
				var datosHMax = [];
				$.getJSON('jsonA.php?' + $(this).serialize(), function(result) {
					$.each(result, function(key, value){
						...
						if (value['y'] > value['y2'])
						{
							Temp = {label: value['label'], y: parseInt(value['y'])};
						}
						TMax = {label: value['label'], y: parseInt(value['y2'])};
						if (value['y3'] > value['y4'])
						{
							Humedad = {label: value['label'], y: parseInt(value['y3'])};
						}
						HMax = {label: value['label'], y: parseInt(value['y4'])};
						datosTemp.push(Temp);
						datosTMax.push(TMax);
					...
					var chart = new CanvasJS.Chart("chartContainer",
					{
						...
						},
						{
							...
							showInLegend: true,            
							dataPoints: datosTMax,
						},{
							type: "column", //change type to bar, line, area, pie, etc
							showInLegend: true,        
							name:"Humedad",
							dataPoints: datosHumedad,
						},
						{
							type: "stepLine",
							axisYIndex: 1,
					...
					chart.render();
					JSONToCSVConvertor(result, "AlertasTyH", true);
				});
			});
		});
	</script>
				...
				<ul class="nav navbar-nav">
					...
					<li class="dropdown">
						...
						<ul class="dropdown-menu">
							...
							<li role="separator" class="divider"></li>
							<li class="active"><a href="alertas.php"><span class="glyphicon glyphicon-warning-sign"></span> Ambas Alertas</a></li>
						</ul>
					</li>
