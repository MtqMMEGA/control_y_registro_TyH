...
$query = "SELECT fecha, humedad, humedad_max FROM registro_errores where fecha >= ".$fechaini." AND fecha <= ".$fechafin." AND humedad > humedad_max ORDER BY id";
$result = mysql_query($query) or die("Error en la instruccion SQL");
$puntos_grafica = array();
while($row = mysql_fetch_array($result))
{        
    $punto = array("label" => $row['fecha'] , "y" => $row['humedad'] , "y2" => $row['humedad_max']);
